...
//			>> pTSKILL->m_dwDurationInc								///적용시간증가비율	stupid Konst stupid Konst 
//		
//
...
//			>> pTSKILL->m_bActive									///패시브=0 액티브=1stupid stupid Konst 
//
...
void CTChart::InitTSKILLTEMP( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		ar	>> pTSKILL->m_strNAME
			>> pTSKILL->m_wSkillID									///스킬ID									///스킬이름
			>> pTSKILL->m_wActSkillID								///이전성공스킬
			>> pTSKILL->m_wNeedItemID
			>> pTSKILL->m_dwClassID						///소모아이템
			>> pTSKILL->m_wDefSkillID								///대상이걸린버프								///직업코드
			>> pTSKILL->m_bKind										///특성구분
			>> pTSKILL->m_bPositive									///이로움 해로움
			>> pTSKILL->m_bLevel									///요구레벨
			>> pTSKILL->m_bLevelInc									///요구레벨증가
...
/*void CTChart::InitTSKILLTEMP( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		>> pTSKILL->m_strNAME         ///?sLlRE름
		>> pTSKILL->m_wActSkillID        ///RER?s??sLl
		>> pTSKILL->m_wNeedItemID        ///LN?lCRELU
		>> pTSKILL->m_wDefSkillID        ///대상RE걸린a置?
		>> pTSKILL->m_dwClassID         ///직l苔湄l
		>> pTSKILL->m_bKind          ///CZLs구s?
		>> pTSKILL->m_bPositive         ///RE로zn ?로zn
		>> pTSKILL->m_bLevel         ///z峨막as?
		>> pTSKILL->m_bLevelInc         ///z峨막as㎲o?
		>> pTSKILL->m_bMaxLevel         ///A獵?sLl?s?
		>> pTSKILL->m_fPrice         ///?격snR?
		>> pTSKILL->m_bDuraSlot         ///l뺑린?N??롯(A煞?)
		>> pTSKILL->m_dwWeaponID        ///사z?ケ?
		>> pTSKILL->m_wUseHP         ///HPLN?snR?
		>> pTSKILL->m_wUseMP         ///MPLN?snR?
		>> pTSKILL->m_bHitInit         ///Ls?R?AE기?(A煞?)
		>> pTSKILL->m_bHitInc         ///Ls?R????(A煞?)
		>> pTSKILL->m_dwSpellTick        ///a森z?A?
		>> pTSKILL->m_bIsRide         ///a置좋?L?L?lr는 ?sLl
		>> pTSKILL->m_dwDelay         ///R瀯?遊諭?
		>> pTSKILL->m_nDelayInc         ///R瀯??대기 ?감snR?
		>> pTSKILL->m_dwGroupTick        ///?z­대기
		>> pTSKILL->m_dwInterval        ///a?a?A?
		>> pTSKILL->m_bDelayType        ///?L?uz?
		>> pTSKILL->m_wModeID         ///L?L
		>> pTSKILL->m_bTargetType        ///사z遊六?피lC
		>> pTSKILL->m_bRangeType        ///대상C??
		>> pTSKILL->m_fMinRange         ///A?N사정?리
		>> pTSKILL->m_fMaxRange         ///A獵六瑩ㅁ綬®
		>> pTSKILL->m_fAtkRange         ///R??Cr,a腑c
		>> pTSKILL->m_fBufRange         ///zR라a??
		>> pTSKILL->m_dwDuration        ///Ruz?A?AE기?
		>> pTSKILL->m_dwDurationInc        ///Ruz?A???snR? 
		>> pTSKILL->m_bCanCancel        ///사z??A?N ?능 z??A煞?)
		>> pTSKILL->m_bContinue         ///?RU지L???
		>> pTSKILL->m_wIconID         ///lCRE콘ID
		>> pTSKILL->m_wItemID[0]        ///a澁?CRELU1
		>> pTSKILL->m_wItemID[1]        ///a澁?CRELU2
		>> pTSKILL->m_bActive         ///C?Ase=0 l?Lse=1
		>> pTSKILL->m_bLoop          ///R湄za?az??
		>> pTSKILL->m_bActionID[TSKILLACTION_SPELL]    ///주a®?RU
		>> pTSKILL->m_bActionID[TSKILLACTION_ACTIVATE]   ///a森z?RU
		>> pTSKILL->m_bActionID[TSKILLACTION_MOVEACTIVATE]  ///RE?a森z?RU
		>> pTSKILL->m_bActionID[TSKILLACTION_DEFEND]   ///acl諍zRU
		>> pTSKILL->m_dwInfoID         ///LrC?D
		>> pTSKILL->m_dwSFX[TSKILLSFX_SPELL]     ///?AR?EClC®
		>> pTSKILL->m_dwSFX[TSKILLSFX_ACT]      ///a森zREClC®
		>> pTSKILL->m_dwSFX[TSKILLSFX_DEFEND]     ///L린UREClC®
		>> pTSKILL->m_dwSFX[TSKILLSFX_DEFENDACT]    ///L린Ua森zREClC®
		>> pTSKILL->m_dwSFX[TSKILLSFX_LOOP]      ///RZ지REClC®
		>> pTSKILL->m_dwSFX[TSKILLSFX_END]      ///?제REClC®
		>> pTSKILL->m_dwSFX[TSKILLSFX_ARROW]     ///a澁?LREClC®
		>> pTSKILL->m_bShowIcon         ///lCRE콘s??
		>> pTSKILL->m_bShowTime         ///l?Rs?A?s??
		>> pTSKILL->m_bShowCritical        ///L㈇췈L?Ruz?
		>> pTSKILL->m_bUseInHold        ///갇Cu 상L??L­ 사z??능z??
		>> pTSKILL->m_bStaticWhenDie       ///사망?A a置?RZ지
		>> pTSKILL->m_fMoveDist        ///Lr?RE? ?리
		>> unused
		>> unused
		>> unused
		>> unused

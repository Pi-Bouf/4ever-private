...
{
	//Font
	//지정ÇN CuC®°? lr큊면 s罐dRÇ CuC®를 상LÓ aT는큊.
...
HRESULT TComponent::DrawText()
{
	...
	if( m_strText != m_strPrvText )
	{
		...
		m_bDidShrink = FALSE; // aRz? UpdateTextOutputDataz?L­ m_bDidShrinkRÇ TRUE를 큊?A 결정ÇN큊.
	}
...
void TComponent::ShrinkText( CString& text)
{
	...
	m_bDidShrink = TRUE; // 켝R微? 짤리면 마ze?s훭L­를 zA려luR뻑§ 풀LR?sC®°? lSzR는 기능R» C캪sC­?AL´
}
...
{
	// a®단lS눔('\n')과 szCTRL.cy 켧REz? 맞게 strORG a췙美¦ R渗óL­
	// Aa햁sÎLÍ 마지막햁까지 outResultz? LrL­대로 큑는큊.
			...
			if( szSIZE.cx > szCTRL.cx )
			{
				...
				while(!strLEFT.IsEmpty())
				{
					...
					else
					{
						...
						if( sizeWORD.cx > szCTRL.cx ) // ÇN 단lî 켧RE°? 컴C÷lÍC췙Ç °?로켧REs릿U 켧큊. RE·?땐 aチ떠Ç R美L기.
						{
							CSize sizeLINE;
							...
							// strNewWord만 strLINEz? C颱Ô, lS머햞 strLEFT로 되킳lC감.
	...
	else
	{
		...
		while(!strTEXT.IsEmpty())
		{
			...
			while(!strTEXT.IsEmpty())
			{
				...
				while( szTEXT.cx > szCTRL.cx ) // 켝R美¦ ÇN컇lz 빼면L­ szCTRL.cy lCz? 들l載A 때까지 R美A큊.
				{
...
VOID TComponent::UpdateTextOutputData( VECTORSTRING& vTextLine, const CSize szCTRL, TextOutputDataSet& outRESULT )
{
	if( !vTextLine.empty() )
	{
		...
		int nLINE = INT(min( szCTRL.cy, szTEXT.cy) * 1.2f);			// ÇN 햁RÇ lôRE(ÇCLz)
		int nTEXT = INT(vTextLine.size());							// A綏쪄Rl森É RüAL 라RÎLö
		nLINE = min( szCTRL.cy, nLINE);

		int nSLOT = min(szCTRL.cy/nLINE, nTEXT);					// A綏쪄N Lö R獵Â A逞l 라RÎLö

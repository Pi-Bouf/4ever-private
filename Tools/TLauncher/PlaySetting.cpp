...
CGameSetting* g_pdlgGameSetting = NULL;
...
CPlaySetting::CPlaySetting(CWnd* pParent /*=NULL*/)
	...
	, m_bClickMove(FALSE)
{
	// GameSetting 다이얼로그 생성 및 설정
	g_pdlgGameSetting = new CGameSetting();
	g_pdlgGameSetting->Create(IDD_SETTING);
	
	g_pdlgGameSetting->ShowWindow(SW_HIDE);
	g_pdlgGameSetting->m_pdlgPlaySetting = this;
}
...
void CPlaySetting::OnBnClickedBtnSetTab()
{
	...
	g_pdlgGameSetting->SetWindowPos(0,rect.left, rect.top, 0,0, SWP_NOSIZE | SWP_SHOWWINDOW);
	ShowWindow(SW_HIDE);
...
void CPlaySetting::OnDestroy()
{
	...
	if(g_pdlgGameSetting)
	{
		delete g_pdlgGameSetting;
		g_pdlgGameSetting = NULL;
	}

...
#pragma region IMAGES
	if (vComp.m_vImageID[0] != LIKE_NULL && vComp.m_vImageID[0] != 0) {
		attributes.push_back(NodeAttribute::FromNameAndValue("DEFAULT", defineValues->FindWithCriteria(vComp.m_vImageID[0], "ID_IMAGE", "ID_IMAGE")));
	}
	if (vComp.m_vImageID[1] != LIKE_NULL && vComp.m_vImageID[1] != 0) {
		attributes.push_back(NodeAttribute::FromNameAndValue("DISABLE", defineValues->FindWithCriteria(vComp.m_vImageID[1], "ID_IMAGE", "ID_IMAGE")));
	}
#pragma endregion

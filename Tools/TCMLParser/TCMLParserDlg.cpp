...
// When TRUE the tool runs head-less (command-line batch); suppress all
// modal message boxes so an automated decompile does not block on a click.
bool g_bBatch = false;
...
void CTCMLParserDlg::LoadHeaders()
{
	...
	if (pFILE == NULL) {
		if (!g_bBatch) AfxMessageBox("Can't open TClient.h...");
		else OutputDebugStringA("TCMLParser: can't open header (symbols will be UNKNOWN_*)\n");
		return;
	}
	else
	{
...
void CTCMLParserDlg::LoadFrames()
{
	...
	if (!g_bBatch) AfxMessageBox("ENDED !");
	else OutputDebugStringA("TCMLParser: decompile ENDED\n");
}
...
Node CTCMLParserDlg::ConvertFrame(FILE *pFILE, bool isFrame, int level)
{
	...
	fread(&pFRAME->m_vCOMP.m_vImageID, sizeof(DWORD), 2, pFILE);

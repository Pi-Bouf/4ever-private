========================================================================
    액티브 템플릿 라이브러리 : TPatchSvr 프로젝트 개요
========================================================================

사용자가 서비스(EXE)를 만드는 데 있어 출발점으로 사용하도록
응용 프로그램 마법사에서 이 TPatchSvr 프로젝트를 만들었습니다.
...
TPatchSvr.idl
    ...
    이 파일은 MIDL 컴파일러에 의해 처리되어 다음을 생성합니다.
        C++ 인터페이스 정의 및 GUID 선언 (TPatchSvr.h)
        GUID 정의                                (TPatchSvr_i.c)
        형식 라이브러리                                  (TPatchSvr.tlb)
        마샬링 코드                                 (TPatchSvr_p.c 및 dlldata.c)

TPatchSvr.h
    이 파일에는 TPatchSvr.idl에서 정의된 항목의 C++ 인터페이스가 정의되어 있고
...
StdAfx.h 및 StdAfx.cpp는
    TPatchSvr.pch라는 이름의 PCH(미리 컴파일된 헤더) 파일과
...
TPatchSvrps.vcproj
	...
	프록시/스텁 DLL을 빌드하는 데 필요한 dlldata.c, TPatchSvr_i.c 및
	TPatchSvr_p.c가 생성됩니다.

...
public:
	...
	void SendCT_ANNOUNCEMENT_ACK(CString strAnnounce); // 현승룡 CT_ANNOUNCEMENT_ACK
	void SendCT_USERKICKOUT_ACK(CString strUser); // 현승룡 CT_USERKICKOUT_ACK
	void SendCT_USERMOVE_ACK(CString strUser, BYTE bChannel, WORD wMapID, FLOAT fPosX, FLOAT fPosY, FLOAT fPosZ); // 현승룡 CT_USERMOVE_ACK
	void SendCT_USERPOSITION_ACK(CString strUser, CString strTarget); // 현승룡 CT_USERPOSITION_ACK
	void SendCT_MONSPAWNFIND_ACK(DWORD dwManager, BYTE bChannel, WORD wMapID, WORD wSpawnID); // 현승룡 CT_MONSPAWNFIND_ACK
	...
	void SendCT_CHATBAN_REQ(CString strName, WORD wMin, DWORD dwBanSeq,DWORD dwManagerID); // 특정 캐릭터 채팅 금지 요청.

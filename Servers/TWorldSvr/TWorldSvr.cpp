...
{
	string dsn, user, passwd;
	int port = DEF_WORLDPORT, grp = 1, sid = 1;

	LoadStringFromIni("DSN", string("TGAME_GSP"), dsn);
	LoadStringFromIni("DBUser", string("sa"), user);
	LoadStringFromIni("DBPasswd", string("1234"), passwd);
	LoadIntFromIni("WorldPort", DEF_WORLDPORT, port);
	LoadIntFromIni("GroupID", 1, grp);
	LoadIntFromIni("ServerID", 1, sid);

	strncpy(m_szDSN, dsn.c_str(), ONE_KBYTE - 1);
	strncpy(m_szDBUserID, user.c_str(), ONE_KBYTE - 1);
	strncpy(m_szDBPasswd, passwd.c_str(), ONE_KBYTE - 1);
	m_wPort = (WORD)port;
	m_bGroupID = (BYTE)grp;
	m_bServerID = (BYTE)sid;

...
/// <summary>
/// Async TCP acceptor for inbound manager (GM tool) peers. Replaces the C++ IOCP accept thread. Each
/// accepted socket becomes a <see cref="PacketConnection"/> handled on its own task; framed packets are
/// handed to <c>onPacket</c> for enqueue onto the control server's batch task.
/// </summary>
public sealed class PacketServer
{
        ...
        CancellationToken token)
    {
        ...
        _logger.LogInformation("Control server listening on port {Port}.", port);
        ...
        CancellationToken token)
    {
        ...
        _logger.LogInformation("Manager connected: {Endpoint}", conn.RemoteEndPoint);
        ...
        finally
        {
            ...
            _logger.LogInformation("Manager disconnected: {Endpoint}", conn.RemoteEndPoint);
        }
    }
}

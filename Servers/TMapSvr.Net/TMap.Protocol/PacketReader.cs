...
public sealed class PacketReader
{
    ...
    /// <summary>Reads <paramref name="count"/> raw bytes (zero-filled past the end of the packet).</summary>
    public byte[] ReadBytes(int count)
    {
        if (count <= 0) return Array.Empty<byte>();
        var dst = new byte[count];
        int avail = Math.Min(count, Math.Max(0, _end - _pos));
        if (avail > 0) Array.Copy(_buf, _pos, dst, 0, avail);
        _pos += count;
        return dst;
    }

    /// <summary>Returns the still-unread body bytes verbatim (used to forward trailing blobs).</summary>

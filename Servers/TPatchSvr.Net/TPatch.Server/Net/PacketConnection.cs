...
/// <summary>
/// One inbound client/control socket. The patch link is <b>plaintext</b> — no cipher, an 8-byte header.
/// The receive loop frames packets and hands each to <c>onPacket</c> (which enqueues to the single batch
/// task); a serialized send loop writes outbound packets. <see cref="RequestClose"/> lets a handler drop
/// the connection after replying (the C++ <c>EC_SESSION_EXIT</c> path used by PATCHSTART / PREPATCHCOMPLETE).
/// </summary>
public sealed class PacketConnection : IAsyncDisposable
{
    ...
    /// <summary>Request an orderly close (drains the pending send, then tears down). Thread-safe.</summary>

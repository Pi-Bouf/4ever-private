...
// ---- Load-time bulk-quest staging --------------------------------------------------------------
// Populated once from whole-table SELECTs at startup, then cleared. Lets LoadQuestTemp build the
// quest graph (and the spawn loop build its monster lists) from memory instead of firing one query
// per parent row. See CTMapSvrModule::Load / LoadQuestTemp.
struct QLOADCHILD	{ DWORD m_dwQuestID; DWORD m_dwTriggerID; BYTE m_bTriggerType; BYTE m_bForceRun; BYTE m_bCountMax; BYTE m_bType; BYTE m_bConditionCheck; };
struct QLOADCOND	{ DWORD m_dwConditionID; BYTE m_bConditionType; BYTE m_bCount; };
struct QLOADREWARD	{ DWORD m_dwRewardID; BYTE m_bRewardType; BYTE m_bTakeMethod; BYTE m_bTakeData; BYTE m_bCount; };
struct QLOADTERM	{ DWORD m_dwTermID; BYTE m_bTermType; BYTE m_bCount; };
struct QMAPMONROW	{ WORD m_wMonID; BYTE m_bLeader; BYTE m_bEssential; BYTE m_bProb; };
struct QMONITEMROW	{ BYTE m_bChartType; WORD m_wItemID; WORD m_wItemIDMin; WORD m_wItemIDMax; WORD m_wWeight; BYTE m_bLevelMin; BYTE m_bLevelMax; BYTE m_bItemProb[MIP_COUNT]; BYTE m_bItemMagicOpt; BYTE m_bItemRareOpt; };
struct QDESTROW		{ WORD m_wDestID; DWORD m_dwPrice; BYTE m_bEnable; BYTE m_bCondition[PORTALCONDITION_COUNT]; DWORD m_dwConditionID[PORTALCONDITION_COUNT]; };

// Load-time server-cell topology: (mapID, unitID, logical-channel) -> owning serverID. Built once from
// the TSVRCHART x TCHANNELCHART join (see CTBLSvrTopoAll) to replace the per-cell TGetServerID proc.
typedef map< __int64, BYTE >				MAPSVRTOPO, *LPMAPSVRTOPO;
inline __int64 MakeSvrTopoKey( WORD wMapID, WORD wUnitID, BYTE bLogChannel)
{ return (__int64(MAKELONG( wMapID, wUnitID)) << 8) | bLogChannel; }

typedef map< DWORD, vector<QLOADCHILD> >	MAPQLOADCHILD;		// quest children keyed by parentID
typedef map< DWORD, vector<QLOADCOND> >		MAPQLOADCOND;		// quest conditions keyed by questID
typedef map< DWORD, vector<QLOADREWARD> >	MAPQLOADREWARD;		// quest rewards keyed by questID
typedef map< DWORD, vector<QLOADTERM> >		MAPQLOADTERM;		// quest terms keyed by questID

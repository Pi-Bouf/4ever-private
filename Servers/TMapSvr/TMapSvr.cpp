...
void main()
{
	...
	string sExit;
	cin >> sExit;
	sExit == "/exit" ? _AtlModule.ExitServer() : main();
}

CTMapSvrModule::CTMapSvrModule()
	: TServerSystem("TMap", TMAP_VERSION)
{
...
DWORD CTMapSvrModule::OnEnter()
{
	...
	SSLogInfo("Loading configuration...");
	DWORD dwResult = LoadConfig();
	if(dwResult)
		return dwResult;


	SSLogInfo("Initializing database...");
	dwResult = InitDB();
	if(dwResult)
		return dwResult;

	SSLogInfo("Loading data...");
	dwResult = LoadData();
	if(dwResult)
		return dwResult;

	SSLogInfo("Creating threads...");
	dwResult = CreateThreads();
	if(dwResult)
		return dwResult;

	SSLogInfo("Initializing network...");
	dwResult = InitNetwork();
	if(dwResult)
		return dwResult;
	SSLogInfo("Listening on port %d.", (int)m_wGamePort);
...
{
	string s; int n;
	LoadStringFromIni("GameDSN", string("TGAME_GSP"), s);  lstrcpyA(m_szGameDSN, s.c_str());
	LoadStringFromIni("DBUser", string("sa"), s);          lstrcpyA(m_szDBUserID, s.c_str());
	LoadStringFromIni("GamePasswd", string("1234"), s);    lstrcpyA(m_szGamePasswd, s.c_str());
	LoadStringFromIni("WorldIP", string("127.0.0.1"), s);  lstrcpyA(m_szWorldIP, s.c_str());
	LoadStringFromIni("LogIP", string("127.0.0.1"), s);    lstrcpyA(m_szLogServerIP, s.c_str());
	LoadIntFromIni("WorldPort", 3816, n); m_wWorldPort = (WORD)n;
	LoadIntFromIni("GamePort", 5816, n);  m_wGamePort = (WORD)n;
	LoadIntFromIni("GroupID", 1, n);      m_bGroupID = (BYTE)n;
	LoadIntFromIni("ServerID", 1, n);     m_bServerID = (BYTE)n;
	LoadIntFromIni("LogPort", 7000, n);   m_wLogServerPORT = (WORD)n;
	DisplayIni();
	return EC_NOERROR;
}

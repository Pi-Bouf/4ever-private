...
DWORD CTLoginSvrModule::OnEnter()
{
	...
	if(dwResult)
		return dwResult;
	SSLogInfo("Listening on port %d.", (int)m_wPort);
...
{
	string s; int n;
	LoadStringFromIni("DSN", string("TGLOBAL_GSP"), s);  lstrcpyA(m_szDSN, s.c_str());
	LoadStringFromIni("DBUser", string("sa"), s);        lstrcpyA(m_szDBUserID, s.c_str());
	LoadStringFromIni("DBPasswd", string("1234"), s);    lstrcpyA(m_szDBPasswd, s.c_str());
	LoadIntFromIni("Port", 4816, n); m_wPort = (WORD)n;
